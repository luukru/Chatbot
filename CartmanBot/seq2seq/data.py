...
'''
def index_chat_input_(vocab_size, freq_dist):
    # get vocabulary of 'vocab_size' most used words
    vocab = freq_dist.most_common(vocab_size)
    # index2word
    index2word = ['_'] + [UNK] + [ x[0] for x in vocab ]
...
def synonyms(sentence, vocab):
    for word in sentence:
        if word not in vocab:
            ...
            if len(synonyms) > 0:
              for syn in synonyms:
                  if syn in vocab and syn is not word:
                      sentence[sentence.index(word)] = syn
                      break
    return sentence

def process_input(input_, vocab, metadata):
    # change to lower case
    input_ = input_.lower()

    # filter out unnecessary characters
    input_ = filter_line(input_, EN_WHITELIST)

    # Tokenize input
    input_tokenized = [ w.strip() for w in input_.split(' ') if w ]

    # Try to replace 'unknown' words with synonyms from the vocabulary.
    input_tokenized = synonyms(input_tokenized, vocab)

    # TODO: check if this makes a difference
    
    # Indexing -> idx2w, w2idx 
    _, w2idx, _ = index_chat_input_(len(vocab), metadata['freq_dist']) 
    idx_q, _ = zero_pad([input_tokenized], [input_tokenized], w2idx)    
