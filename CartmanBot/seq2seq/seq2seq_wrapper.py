...
class Seq2Seq(object):
    ...
    #    prints statistics
    def train(self, train_set, valid_set, sess=None, starting_iteration=0):
        ...
        for i in range(self.epochs):
            try:
                ...
                if i and i% (self.epochs//100) == 0: # TODO : make this tunable by the user
                    # save model to disk
                    saver.save(sess, self.ckpt_path + self.model_name + '.ckpt', global_step=i + starting_iteration)
                    # evaluate to get validation loss
                    val_loss = self.eval_batches(sess, valid_set, 16) # TODO : and this
                    # print stats
                    print('\nModel saved to disk at iteration #{}'.format(i + starting_iteration))
                    print('val   loss : {0:.6f}'.format(val_loss))
                    sys.stdout.flush()
            except KeyboardInterrupt: # this will most definitely happen, so handle it
                print('Interrupted by user at iteration {}'.format(i + starting_iteration))
                self.session = sess
                return sess

    # Restore previous sessions
    def restore_last_session(self, file_name=None):
        saver = tf.train.Saver()
        # create a session
        sess = tf.Session()
        # get checkpoint state
        if file_name != None:
            ckpt = tf.train.get_checkpoint_state(self.ckpt_path, file_name)
        else:
